...
sales_agent = Agent(
    ...
    instructions="Ты - агент по продаже товаров. Помогай пользователю с покупками.\n"
                 ...
                 "При получении подтверждения от пользователя о желании сделать заказ, ОБЯЗАТЕЛЬНО спроси его username "
                 "(никнейм/имя пользователя). Без username заказ создать нельзя.\n"
                 "После получения username создай заказ при помощи функции create_order, используя полученный username, "
                 "идентификатор товара и количество.\n"
...
refund_agent = Agent(
    ...
    instructions="Ты - агент по возврату денег. Помогай пользователю с возвратом средств.\n"
                 ...
                 "При получении подтверждения от пользователя о желании сделать возврат, ОБЯЗАТЕЛЬНО спроси его username "
                 "(никнейм/имя пользователя), который использовался при создании заказа.\n"
                 "После получения username ты можешь найти заказ в таблице командой find_order с указанием id товара "
                 "и полученного username пользователя.\n"
...
change_agent = Agent(
    ...
    instructions="Ты - агент по изменению заказов. Помогай пользователю с изменением заказов, если они захотели"
                 ...
                 "Получи от пользователя описание товара из заказа и покажи список товаров функцией show_items.\n"
                 "ОБЯЗАТЕЛЬНО спроси username (никнейм/имя пользователя) пользователя, который использовался при "
                 "создании заказа.\n"
                 "После получения username найди заказ командой find_order с использованием полученного username и "
                 "идентификатора товара.\n"
                 "Если такой заказ найден, тебе нужно получить новое количество товаров для отмены. "
                 "Количество для отмены ДОЛЖНО быть меньше количества товаров из заказа.\n"
